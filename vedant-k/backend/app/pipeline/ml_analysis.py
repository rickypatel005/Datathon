...
from backend.app.pipeline.statistical import run_statistical_analysis, sanitize_for_json
...
from backend.app.pipeline.experiments import run_controlled_experiments


def determine_ml_task(discovery: DiscoveryContract) -> MLTaskType:
    """
    ...
    """
    if discovery.router.classification and discovery.target_candidates:
        return MLTaskType.CLASSIFICATION
    elif discovery.router.regression and discovery.target_candidates:
        return MLTaskType.REGRESSION
    elif discovery.router.time_analysis:
        return MLTaskType.TIME_ANALYSIS
    elif discovery.router.clustering:
        return MLTaskType.CLUSTERING
    elif discovery.router.anomaly_detection:
    ...
    if isinstance(discovery, dict):
        try:
            ...
            return AnalysisContract(
                schema_version="1.0",
                status=PipelineStatus.FAILED,
                reason=f"Invalid DiscoveryContract schema: {str(exc)}",
                task=MLTaskType.NOT_APPLICABLE,
                validation=ValidationReport(status=PipelineStatus.FAILED, reason="DiscoveryContract schema validation error"),
                statistical_analysis=StatisticalReport(status=PipelineStatus.FAILED, reason="DiscoveryContract schema validation error"),
                winner=ChampionResult(status=PipelineStatus.FAILED, reason="DiscoveryContract schema validation error"),
                error_analysis=ErrorAnalysisReport(status=PipelineStatus.FAILED, reason="DiscoveryContract schema validation error"),
                ensemble=EnsembleReport(status=PipelineStatus.FAILED, reason="DiscoveryContract schema validation error"),
                time_analysis=TimeAnalysisReport(status=PipelineStatus.FAILED, reason="DiscoveryContract schema validation error"),
            )
    elif not isinstance(discovery, DiscoveryContract):
        return AnalysisContract(
            schema_version="1.0",
            status=PipelineStatus.FAILED,
            reason=f"Invalid DiscoveryContract: expected DiscoveryContract or dict, got {type(discovery).__name__}",
            task=MLTaskType.NOT_APPLICABLE,
